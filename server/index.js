...
io.on('connection', socket => {
    ...
    socket.on('start draw', data => {
        setTimeout(() => { socket.broadcast.emit('start draw', data) }, 1000);
    })
    socket.on('draw', (data) => {
        setTimeout(() => { socket.broadcast.emit('draw', data) }, 1000);
    })
    socket.on('finish draw', () => {
        setTimeout(() => { socket.broadcast.emit('finish draw',) }, 1000);
    })
    socket.on('clear', () => {
        setTimeout(() => { socket.broadcast.emit('clear') }, 1000);
    })
})
